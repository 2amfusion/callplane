...
5. **Health:** `health-wrapper` on Railway `$PORT` (`GET /` → 200); livekit/sip `health_port` is **8081** (set by entrypoint). `livekit-sip/railway.json` explicit startCommand starts wrapper then entrypoint.
