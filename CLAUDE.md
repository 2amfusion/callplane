...
- SIP Railway: `GET $PORT/` = health-wrapper (must see `[health-wrapper] Listening` in logs). Real SIP: `service ready` then `curl :8081/` → 200. `livekit-sip/railway.json` **`startCommand`** = `/start.sh`. Wrong Root Directory or SFU startCommand → instant health fail. Do not set manual `PORT=8080` unless debugging — Railway injects `$PORT`. Troubleshooting: `livekit-sip/VERIFY-DEPLOY.md`.
