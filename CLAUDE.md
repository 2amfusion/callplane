...
- SIP Railway: `GET $PORT/` = health-wrapper (Deploy **runtime** logs must show `[start.sh]` then `[health-wrapper] Listening` — all startup logs go to **stdout**, not stderr). Real SIP: `service ready` then `curl :8081/` → 200. `railway.json` **`startCommand`: null** (Dockerfile `ENTRYPOINT /start.sh`). Wrong Root Directory or dashboard **startCommand** `livekit-server …` → instant health fail. `HEALTH_ONLY=1` isolates wrapper vs SIP config. Do not set manual `PORT=8080`. Troubleshooting: `livekit-sip/VERIFY-DEPLOY.md` §10.
- **callplane-agents Railway:** same health-wrapper pattern as SIP — `start.sh` binds `$PORT`, worker HTTP on **`AGENT_HTTP_PORT` (8081)**. Wrong Root Directory or sticky **`python agent.py start`** startCommand → health fail (nothing on `$PORT`). Missing `LIVEKIT_URL` / `LIVEKIT_API_KEY` / `LIVEKIT_API_SECRET` crashes worker after wrapper is up. Do not set manual `PORT`.
